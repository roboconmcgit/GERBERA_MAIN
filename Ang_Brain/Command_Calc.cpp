...
float CommandCalc::LineTracer(int line_value,float traceforward) {
	...
	const float KP=LineTracer_KP,KI=LineTracer_KI,KD=LineTracer_KD;			//PIDゲインの設定
...
void CommandCalc::LookUpGateRunner(int line_value_lug, float odo, float angle,int line_value){
	...
	float y_t;
	  ...
	  case Approach_to_LUG:
	    ...
	    if((gClock->now() - clock_start) > (uint32_t)Approach_to_LUG_time){
	  ...
	  case LUG_Tail_On:
		    forward = LUG_Tail_On_fowrd;
		    yawratecmd = 0;
		    if((gClock->now() - clock_start) > (uint32_t)LUG_Tail_On_time){
		      forward = 0;
		      yawratecmd = 0;
		      tail_mode_lflag = true;
